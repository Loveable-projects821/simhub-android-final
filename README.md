...
## Test if YOUR phone can capture real call audio (do this before anything else)
1. Build and install as above.
2. Grant the microphone permission when the app asks (tap "Test Call Audio Capture"
   once — it'll prompt).
3. Get a **real phone call actually connected** — call the device from another phone,
   answer it, have the other person talk continuously.
4. While the call is live, tap **"Test Call Audio Capture"** in the app.
5. Keep both people talking for the ~12 seconds it takes to test 4 audio sources.
6. Read the result on screen:
   - `init=false` for a source → blocked outright on this device for that source.
   - `init=true gotAudio=false avgAmp=<small number, usually 0-10>` → it opened the
     mic but got silence — also blocked, just less obviously.
   - `init=true gotAudio=true avgAmp=<bigger number, generally 100+>` → **this device
     actually lets us capture real call audio.** This is the one you want to see.

If **any** source shows `gotAudio=true`, tell me the exact source name and avgAmp
number — I'll wire that source into the live WebRTC audio path next, and real
caller audio on iPhone becomes achievable on this phone specifically.

If **all four** show `gotAudio=false`, this device's manufacturer has fully blocked
it (common on Pixel and recent Samsung) — root access or dedicated modem hardware
become the only remaining options, as discussed.
...
## How to build — Option A: cloud build via GitHub Actions (no Android Studio)
This project already includes `.github/workflows/build.yml`.
1. Create a free GitHub account, make a new repository, upload this entire folder
   (drag-and-drop upload works fine on github.com).
2. Go to the repo's **Actions** tab — a build starts automatically.
3. When it finishes (green check), click into the run → download the
   **simhub-debug-apk** artifact — that's your installable APK.
4. Open that download link on the Android phone itself (or transfer the file over),
   tap it, allow "install from unknown sources" if prompted, done.

## How to build — Option B: Android Studio locally
